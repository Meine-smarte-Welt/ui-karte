...
function sanitizeColor(value) {
  ...
  if (!SAFE_COLOR_RE.test(trimmed)) {
    ...
    console.warn(
      "manuelle-dashboardkarte-card: ungültiger Farbwert ignoriert (nur Hex/rgb()/hsl()/var()/CSS-Farbnamen erlaubt):",
      value
    );
    return "";
  }
...
function sanitizeImageUrl(value) {
  ...
  if (!SAFE_URL_RE.test(trimmed)) {
    // eslint-disable-next-line no-console
    console.warn("manuelle-dashboardkarte-card: ungültige Bild-URL ignoriert:", value);
    return "";
  }
  return trimmed;
}
...
class ManuelleDashboardkarteCard extends HTMLElement {
  constructor() {
    super();
  ...
  setConfig(config) {
    if (!config || typeof config !== "object") {
      throw new Error("manuelle-dashboardkarte-card: Ungültige Konfiguration.");
    }
  ...
  static getConfigElement() {
    return document.createElement("manuelle-dashboardkarte-card-editor");
  }
...
class ManuelleDashboardkarteCardEditor extends HTMLElement {
  constructor() {
    super();
    this._pickers = [];
...
customElements.define("manuelle-dashboardkarte-card", ManuelleDashboardkarteCard);
customElements.define("manuelle-dashboardkarte-card-editor", ManuelleDashboardkarteCardEditor);

window.customCards = window.customCards || [];
window.customCards.push({
  type: "manuelle-dashboardkarte-card",
  name: "Manuelle Dashboardkarte",
  description:
